...
condensata_df$id <- 1:nrow(condensata_df)
...
# par(mfrow = c(1,1))
#   plot(cov_clean[[1]])
#   points(condensata_df, pch = 19, cex = 0.3)
#   points(occ_filt_nbin[,2:3], pch = 19, cex = 0.3, col = 'red')

condensata_filt_pres <- occ_filt_nbin[,2:3]
condensata_filt_pres$pr_ab <- 1
...
condensata_filt_pres$folds <- spat_blocks1$folds_ids
condensata_filt_pres %>%
  group_by(folds) %>%
  count()
...
pa <- lapply(1:k, function(x) {
  sample_pseudoabs(
    data = condensata_filt_pres,
    x = 'lon',
    y = 'lat',
    n = sum(condensata_filt_pres$folds == x),
...
#   geom_point(data = rbind(condensata_filt_pres, pa),
#              aes(
#                x = lon,
#                y = lat,
#                col = as.factor(folds),
...
SWDdata <- prepareSWD(
  species = 'Aristida condensata',
  p = condensata_filt_pres[,1:2],
  a = pa[,1:2],
  env = cov_clean
)
...
spat_blocks2 <- cv_spatial(
  x = st_as_sf(bind_rows(condensata_filt_pres, pa), coords = c('lon', 'lat'), crs = crs(cov_clean)),
  column = 'pr_ab',
  r = cov_clean[[1]],
  k = k,
  hexagon = FALSE,
...
ggplot() +
  ...
  geom_sf(data = st_as_sf(condensata_filt_pres, coords = c('lon', 'lat'), crs = crs(states)), size = 1, col = 'black', fill = 'white', pch = 21) +
