...
am_clean <- read.table(file = "species_data/a_mohrii_occ_clean.csv", header = TRUE, sep = ",")

am_no_crs <- st_as_sf(am_clean, coords = c('lon', 'lat'), remove = FALSE)
am_crs84 <- am_no_crs
st_crs(am_crs84) = 4326
...
# Calibration Area --------------------------------------------------------
# calib_am <- calib_area (data = as.data.frame(am_crs84),
#                         x = 'lon', y = 'lat',
#                         method = c('buffer', width = 150000),
#                         crs = crs(am_crs84))

# aoi <- terra::intersect(calib_am, states)

# plot(calib_am)
# plot(aoi, col = 'red', add = TRUE)
# plot(states, add = TRUE)
# plot(am_crs84, add = TRUE)
...
# mohrii_filt_pres <- read.table(file = "species_data/a_mohrii_spatial_blocks.csv", header = TRUE, sep = ",")

k = 5
# Spatial Block Cross-Validation ------------------------------------------

mohrii_df <- as.data.frame(am_crs84) %>%
  (dplyr::select)(lon, lat)

mohrii_df$id <- 1:nrow(mohrii_df)
...
# par(mfrow = c(1,1))
#   plot(cov_clean[[1]])
#   points(mohrii_df, pch = 19, cex = 0.3)
#   points(occ_filt_nbin[,2:3], pch = 19, cex = 0.3, col = 'red')

mohrii_filt_pres <- occ_filt_nbin[,2:3]
mohrii_filt_pres$pr_ab <- 1

spat_range <- cv_spatial_autocor(
  r = cov_clean,
  x = st_as_sf(mohrii_filt_pres, coords = c('lon', 'lat'), crs = crs(cov_clean)),
  column = 'pr_ab',
  plot = TRUE
)
#recommended block size returned 104744
...
mohrii_filt_pres$folds <- spat_blocks1$folds_ids
mohrii_filt_pres %>%
  group_by(folds) %>%
  count()
...
# write.csv(x = mohrii_filt_pres, file = "mohrii_spatial_blocks.csv", sep = ",", quote = FALSE)


# pa <- read.table(file = "species_data/a_mohrii_pseudo_absence_data.csv", header = TRUE, sep = ",")
# Pseudo-Absence Data -----------------------------------------------------
## using proportional stratification
pa <- lapply(1:k, function(x) {
  sample_pseudoabs(
    data = mohrii_filt_pres,
    x = 'lon',
    y = 'lat',
    n = sum(mohrii_filt_pres$folds == x),
...
#   geom_point(data = rbind(mohrii_filt_pres, pa),
#              aes(
#                x = lon,
#                y = lat,
#                col = as.factor(folds),
...
# write.csv(x = pa, file = "a_mohrii_pseudo_absence_data.csv", quote = FALSE)

### Extracting covariate values for each point
SWDdata <- prepareSWD(
  species = 'Aristida mohrii',
  p = mohrii_filt_pres[,1:2],
  a = pa[,1:2],
  env = cov_clean
)
...
spat_blocks2 <- cv_spatial(
  x = st_as_sf(bind_rows(mohrii_filt_pres, pa), coords = c('lon', 'lat'), crs = crs(cov_clean)),
  column = 'pr_ab',
  r = cov_clean[[1]],
  k = k,
  hexagon = FALSE,
...
### model prediction
# terra::writeRaster(x = pred, file = "a_mohrii_model_prediction.tiff") 
...
ggplot() +
  ...
  geom_sf(data = st_as_sf(mohrii_filt_pres, coords = c('lon', 'lat'), crs = crs(states)), size = 1, col = 'black', fill = 'white', pch = 21) +
