...
as_clean <- read.table(file = "a_stricta_occ_clean.csv", header = TRUE, sep = ",")
...
# Calibration Area --------------------------------------------------------
# calib_as <- calib_area (data = as.data.frame(as_crs84),
#                         x = 'lon', y = 'lat',
...
# soil
names(covariates) <- c("clay_0_5cm_mean", "clay_5_15cm", "clay_15_30cm", "nitrogen_0_5cm",
                      "nitrogen_5_15cm", "nitrogen_15_30cm", "ocd_0_5cm", "ocd_5_15cm",
                      "ocd_15_30cm", "soc_0_5cm", "soc_5_15cm", "soc_15_30cm",
                      "phh2o_0_5cm", "phh2o_5_15cm", "phh2o_15_30cm", "sand_0_5cm",
                      "sand_5_15cm", "sand_15_30cm", "silt_0_5cm", "silt_5_15cm",
...
# selected_vars <- c("mean_ann_t","mean_diurnal_t_range",'mean_t_wet_q','mean_t_dry_q','mean_t_warm_q','mean_t_cold_q','p_seas','p_wet_q','p_dry_q','p_warm_q','p_cold_q'
# )

# selected_vars <- c("f_intensity","f_freq","f_seas","mean_ann_t","mean_diurnal_t_range","isothermality","t_seas",'max_t_warm_m','min_t_cold_m',"t_ann_range",'mean_t_wet_q','mean_t_dry_q','mean_t_warm_q','mean_t_cold_q','ann_p','p_wet_m','p_dry_m','p_seas','p_wet_q','p_dry_q','p_warm_q','p_cold_q'
# )

## 0-5cm soil only
selected_vars <- c("clay_15_30cm", "nitrogen_15_30cm","ocd_15_30cm","soc_15_30cm","phh2o_15_30cm","sand_5_15cm","silt_5_15cm"
)

## weather only
# selected_vars <- c("mean_ann_t","mean_diurnal_t_range","isothermality","t_seas",'max_t_warm_m','min_t_cold_m',"t_ann_range",'mean_t_wet_q','mean_t_dry_q','mean_t_warm_q','mean_t_cold_q','ann_p','p_wet_m','p_dry_m','p_seas','p_wet_q','p_dry_q','p_warm_q','p_cold_q'
# )

cov_clean <- covariates[[selected_vars]]

# table with cross-validation
# stricta_filt_pres <- read.table(file = "Aristida_stricta/stricta_filt_pres.csv", header = TRUE, sep = ",")

k = 5
# Spatial Block Cross-Validation ------------------------------------------

stricta_df <- as.data.frame(as_crs84) %>%
  (dplyr::select)(lon, lat)

stricta_df$id <- 1:nrow(stricta_df)
...
# par(mfrow = c(1,1))
#   plot(cov_clean[[1]])
#   points(stricta_df, pch = 19, cex = 0.3)
#   points(occ_filt_nbin[,2:3], pch = 19, cex = 0.3, col = 'red')

stricta_filt_pres <- occ_filt_nbin[,2:3]
stricta_filt_pres$pr_ab <- 1

spat_range <- cv_spatial_autocor(
  r = cov_clean,
  x = st_as_sf(stricta_filt_pres, coords = c('lon', 'lat'), crs = crs(cov_clean)),
  column = 'pr_ab',
  plot = TRUE
)
  #recommended block size returned 161531
...
stricta_filt_pres$folds <- spat_blocks1$folds_ids
stricta_filt_pres %>%
  group_by(folds) %>%
  count()
...
# write.csv(x = stricta_filt_pres, file = "stricta_filt_pres.csv", sep = ",", quote = FALSE)



# pa <- read.table(file = "Aristida_stricta/pseudo_absence_data_stricta.csv", header = TRUE, sep = ",")
# Pseudo-Absence Data -----------------------------------------------------
## using proportional stratification
pa <- lapply(1:k, function(x) {
  sample_pseudoabs(
    data = stricta_filt_pres,
    x = 'lon',
    y = 'lat',
    n = sum(stricta_filt_pres$folds == x),
...
#   geom_point(data = rbind(stricta_filt_pres, pa),
#              aes(
#                x = lon,
#                y = lat,
#                col = as.factor(folds),
...
# write.csv(x = pa, file = "Aristida_stricta/pseudo_absence_data_stricta.csv", sep = ",", quote = FALSE)

### Extracting covariate values for each point
SWDdata <- prepareSWD(
  species = 'Aristida stricta',
  p = stricta_filt_pres[,1:2],
  a = pa[,1:2],
  env = cov_clean
)
...
spat_blocks2 <- cv_spatial(
  x = st_as_sf(bind_rows(stricta_filt_pres, pa), coords = c('lon', 'lat'), crs = crs(cov_clean)),
  column = 'pr_ab',
  r = cov_clean[[1]],
  k = k,
  hexagon = FALSE,
...
# saveRDS(rf_sbcv, "rf_spatial_block_stricta.rds")
...
### Response Curves
plotResponse(rf_sbcv, var = "phh2o_5_15cm", marginal = TRUE, rug = TRUE) + labs(x = 'pH of H20') +
  plotResponse(rf_sbcv, var = "nitrogen_5_15cm", marginal = TRUE, rug = TRUE) + labs(x = 'nitrogen')

  
#ggsave('C:/Users/emars/Desktop/Intro to spatial data in R/response_curves.pdf')

### model prediction
# terra::writeRaster(x = pred, file = "a_stricta_model_prediction.tiff") 
...
ggplot() +
  ...
  geom_sf(data = st_as_sf(stricta_filt_pres, coords = c('lon', 'lat'), crs = crs(states)), size = 1, col = 'black', fill = 'white', pch = 21) +
