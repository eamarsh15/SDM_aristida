...
# par(mfrow = c(1,1))
#   plot(cov_clean[[1]])
#   points(beyrichiana_df, pch = 19, cex = 0.3)
#   points(occ_filt_nbin[,2:3], pch = 19, cex = 0.3, col = 'red')

beyrichiana_filt_pres <- occ_filt_nbin[,2:3]
...
beyrichiana_filt_pres$folds <- spat_blocks1$folds_ids
beyrichiana_filt_pres %>% group_by(folds) %>% count()
...
pa <- lapply(1:k, function(x) {
  sample_pseudoabs(
    data = beyrichiana_filt_pres,
    x = 'lon',
    y = 'lat',
    n = sum(beyrichiana_filt_pres$folds == x),
...
#   geom_point(data = rbind(beyrichiana_filt_pres, pa),
#              aes(
#                x = lon,
#                y = lat,
#                col = as.factor(folds),
...
SWDdata <- prepareSWD(
  species = 'Aristida beyrichiana',
  p = beyrichiana_filt_pres[,1:2],
  a = pa[,1:2],
  env = cov_clean
)
...
spat_blocks2 <- cv_spatial(
  x = st_as_sf(bind_rows(beyrichiana_filt_pres, pa), coords = c('lon', 'lat'), crs = crs(cov_clean)),
  column = 'pr_ab',
  r = cov_clean[[1]],
  k = k,
  hexagon = FALSE,
...
ggplot() +
  ...
  geom_sf(data = st_as_sf(beyrichiana_filt_pres, coords = c('lon', 'lat'), crs = crs(states)), size = 1, col = 'black', fill = 'white', pch = 21) +
