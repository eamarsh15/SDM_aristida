...
ag_clean <- read.table(file = "species_data/a_gyrans_occ_clean.csv", header = TRUE, sep = ",")

ag_no_crs <- st_as_sf(ag_clean, coords = c('lon', 'lat'), remove = FALSE)
ag_crs84 <- ag_no_crs
st_crs(ag_crs84) = 4326
...
# Calibration Area --------------------------------------------------------
# calib_ag <- calib_area (data = as.data.frame(ag_crs84),
#                         x = 'lon', y = 'lat',
#                         method = c('buffer', width = 150000),
#                         crs = crs(ag_crs84))

# aoi <- terra::intersect(calib_ag, states)

# plot(calib_ag)
# plot(aoi, col = 'red', add = TRUE)
# plot(states, add = TRUE)
# plot(ag_crs84, add = TRUE)
...
# gyrans_filt_pres <- read.table(file = "species_data/a_gyrans_spatial_blocks.csv", header = TRUE, sep = ",")

k = 5
# Spatial Block Cross-Validation ------------------------------------------

gyrans_df <- as.data.frame(ag_crs84) %>%
  (dplyr::select)(lon, lat)

gyrans_df$id <- 1:nrow(gyrans_df)
...
# par(mfrow = c(1,1))
#   plot(cov_clean[[1]])
#   points(gyrans_df, pch = 19, cex = 0.3)
#   points(occ_filt_nbin[,2:3], pch = 19, cex = 0.3, col = 'red')

gyrans_filt_pres <- occ_filt_nbin[,2:3]
gyrans_filt_pres$pr_ab <- 1

spat_range <- cv_spatial_autocor(
  r = cov_clean,
  x = st_as_sf(gyrans_filt_pres, coords = c('lon', 'lat'), crs = crs(cov_clean)),
  column = 'pr_ab',
  plot = TRUE
)
#recommended block size returned 113278
...
gyrans_filt_pres$folds <- spat_blocks1$folds_ids
gyrans_filt_pres %>%
  group_by(folds) %>%
  count()
...
# write.csv(x = gyrans_filt_pres, file = "gyrans_spatial_blocks.csv", sep = ",", quote = FALSE)


# pa <- read.table(file = "species_data/a_gyrans_pseudo_absence_data.csv", header = TRUE, sep = ",")
# Pseudo-Absence Data -----------------------------------------------------
## using proportional stratification
pa <- lapply(1:k, function(x) {
  sample_pseudoabs(
    data = gyrans_filt_pres,
    x = 'lon',
    y = 'lat',
    n = sum(gyrans_filt_pres$folds == x),
...
#   geom_point(data = rbind(gyrans_filt_pres, pa),
#              aes(
#                x = lon,
#                y = lat,
#                col = as.factor(folds),
...
# write.csv(x = pa, file = "a_gyrans_pseudo_absence_data.csv", quote = FALSE)

### Extracting covariate values for each point
SWDdata <- prepareSWD(
  species = 'Aristida gyrans',
  p = gyrans_filt_pres[,1:2],
  a = pa[,1:2],
  env = cov_clean
)
...
spat_blocks2 <- cv_spatial(
  x = st_as_sf(bind_rows(gyrans_filt_pres, pa), coords = c('lon', 'lat'), crs = crs(cov_clean)),
  column = 'pr_ab',
  r = cov_clean[[1]],
  k = k,
  hexagon = FALSE,
...
### model prediction
# terra::writeRaster(x = pred, file = "a_gyrans_model_prediction.tiff") 
...
ggplot() +
  ...
  geom_sf(data = st_as_sf(gyrans_filt_pres, coords = c('lon', 'lat'), crs = crs(states)), size = 1, col = 'black', fill = 'white', pch = 21) +
